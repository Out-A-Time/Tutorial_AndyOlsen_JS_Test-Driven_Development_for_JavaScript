describe("Matchers for equality", () => {
 ...
 //tests two references refer to the same object address
 ...
 test("toMatch() matches a string against an RegularExpression Regex", () => {
  const message = "Oslo is in Norway";

  expect(message).toMatch("Norway"); //does it contain word "Norway"
  expect(message).toMatch(/norw../i); //case insensitive
  expect(message).not.toMatch(/norw../); //case sensitive
 });
 ...
 test("toMatchObject() matches object properties", () => {
  ...
  // expect(carDesired).toMatchObject(carOnSale); //this will fail because carDesired doesn't match all properties
 });
});
